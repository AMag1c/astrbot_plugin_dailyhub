...
- **按源会话访问控制**：每个源可独立设名单模式（all / 白名单 / 黑名单），限定哪些会话可用该源的获取指令。
- **三级图片降级**：在线 `html_render` 出图 → 本地 Pillow 卡片 → 纯文字，渲染服务不可用时也有兜底。
...
| 💻 实时 IT 资讯 | `/it资讯`（itnews、it新闻） | `/v2/it-news` | `hot_push_cron`（默认 12、20 点） |
| 🔥 IT 之家热榜 | `/it热搜`（IT热搜） | `/v2/it-news/rank` | `hot_push_cron` |
| 🪙 黄金价格 | `/金价`（黄金、gold、黄金价格） | `/v2/gold-price` | `hot_push_cron` |
| 🎵 抖音热搜 | `/抖音`（douyin、抖音热搜） | `/v2/douyin` | `hot_push_cron` |
| 📕 小红书热搜 | `/小红书`（xhs、小红书热搜） | `/v2/rednote` | `hot_push_cron` |
| 📺 哔哩哔哩热搜 | `/b站`（哔哩哔哩、bilibili、B站、b站热搜、B站热搜） | `/v2/bili` | `hot_push_cron` |
| 🌐 微博热搜 | `/微博`（weibo、微博热搜） | `/v2/weibo` | `hot_push_cron` |
...
| `render_mode` | 出图后端：自动 / API渲染 / 本地渲染 | 自动 |
| `dark_mode` / `list_top_n` | 图片深色主题 / 榜单取前 N 条 | true / 15 |
| `enable_dedup` | 定时推送去重（数据无变化不推） | true |
| `daily_push_time` / `hot_push_cron` | 日级源、热榜的默认推送时间 | 09:00 / `0 12,20 * * *` |
...
- **`access_mode` 名单模式**：限定本源「获取指令」可用范围——`all` 所有会话（默认）/ `whitelist` 仅名单内 / `blacklist` 名单内禁用。不影响推送目标。
- **`targets` 会话名单**：本源的推送目标，同时也是上面名单模式用到的访问名单。点「添加」逐条填会话 UMO（目标会话发 `/sid` 获取），也可用 `/订阅资讯` 在会话内自助订阅。
...
2. 依赖：`aiohttp`、`croniter`、`Pillow`（见 `requirements.txt`，框架会自动安装）。
3. 重启 / 重载 AstrBot。

## ⚠️ 注意事项

- **图片渲染**：默认走 AstrBot 内置的 `html_render`（远程 t2i 服务，需联网）。该服务故障时本插件会自动回退**本地 Pillow 简版卡片**或纯文字——所以即使渲染服务挂了也能出内容。本地渲染需系统装有中文字体。
